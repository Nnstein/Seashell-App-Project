...
using Seashell.Resort.Extensions;

namespace Seashell.Resort.ApiClient
{
    public class AbpApiClient : ISingletonDependency, IDisposable
    {
        ...
        public IFlurlClient GetClient(string accessToken)
        {
            if (_client == null)
            {
                CreateClient();
        ...
        {
            _client= FlurlHttp.Clients.GetOrAdd("_clients", ApiUrlConfig.BaseUrl);
        ...
        private void AddHeaders(string accessToken)
        {
            ...
            _client.WithHeader("User-Agent", Seashell.Resort.AbpZeroTemplateConsts.AbpApiClientUserAgent);
