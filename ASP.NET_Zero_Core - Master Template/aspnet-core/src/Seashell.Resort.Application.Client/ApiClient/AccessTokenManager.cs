...
namespace Seashell.Resort.ApiClient
{
    public class AccessTokenManager : IAccessTokenManager, ISingletonDependency
    {
        ...
        {
            IFlurlClient client = FlurlHttp.Clients.GetOrAdd("_auth_clients", ApiUrlConfig.BaseUrl);
            client.WithHeader("Accept", new MediaTypeWithQualityHeaderValue("application/json"));
            client.WithHeader("User-Agent", Seashell.Resort.AbpZeroTemplateConsts.AbpApiClientUserAgent);
            return client;
        }
    }
}