...
using Seashell.Resort.Authentication.TwoFactor;
using Seashell.Resort.Authentication.TwoFactor.Google;
...
using Seashell.Resort.Web.Authentication.JwtBearer;
using Seashell.Resort.Web.Authentication.TwoFactor;
using Seashell.Resort.Web.Models.TokenAuth;
using Seashell.Resort.Authorization.Impersonation;
using Seashell.Resort.Authorization.Roles;
using Seashell.Resort.Configuration;
using Seashell.Resort.Identity;
using Seashell.Resort.Net.Sms;
using Seashell.Resort.Notifications;
using Seashell.Resort.Security.Recaptcha;
using Seashell.Resort.Web.Authentication.External;
using Seashell.Resort.Web.Common;
    ...
    public class TokenAuthController : AbpZeroTemplateControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(AbpZeroTemplateConsts.DateTimeOffsetFormat));
        ...
        private string GetTenancyNameOrNull()
        {
            ...
            {
                var headerValue = Request.Headers["Abp.TenantId"].FirstOrDefault();
                if (int.TryParse(headerValue, out var parsedTenantId))
                {
