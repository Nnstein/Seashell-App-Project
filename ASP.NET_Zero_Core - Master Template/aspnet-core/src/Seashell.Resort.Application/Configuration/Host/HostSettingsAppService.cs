...
using Seashell.Resort.Timing;
using Seashell.Resort.Timing.Dto;
...
namespace Seashell.Resort.Configuration.Host
{
    ...
    public class HostSettingsAppService : SettingsAppServiceBase, IHostSettingsAppService
    {
        ...
        private async Task CheckUserConfirmationStatus(HostUserManagementSettingsEditDto settings)
        {
        ...
        private async Task UpdateEmailSettingsAsync(EmailSettingsEditDto settings)
        {
            ...
            var useDefaultCredentials = (!settings.SmtpUseAuthentication).ToString().ToLowerInvariant();
