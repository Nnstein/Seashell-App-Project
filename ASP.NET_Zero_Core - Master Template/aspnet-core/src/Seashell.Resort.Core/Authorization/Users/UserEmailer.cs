...
using Seashell.Resort.Localization;
...
using Abp.Timing;
using Seashell.Resort.Authorization.PasswordlessLogin;
using Seashell.Resort.Configuration;
using Seashell.Resort.Net.Emailing;
...
namespace Seashell.Resort.Authorization.Users
{
    ...
    public class UserEmailer : AbpZeroTemplateServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendPasswordResetLinkAsync(User user, string link = null)
        {
            ...
            if (!link.IsNullOrEmpty())
            {
                ...
                var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                    .ToString(AbpZeroTemplateConsts.DateTimeOffsetFormat, CultureInfo.InvariantCulture));
        ...
        public async Task SendPasswordlessLoginCodeAsync(User user, string code)
        {
            ...
            mailMessage.AppendLine("<br />" + L("PasswordlessCode_Description") );
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<br />");
            
            mailMessage.AppendLine(
                $@"<table cellpadding='0' cellspacing='0' width='100%' style='border-collapse:separate;border-spacing:0px;border-left:2px dashed #e7ecee;border-right:2px dashed #e7ecee;border-top:2px dashed #e7ecee;border-bottom:2px dashed #e7ecee;border-radius:36px' role='presentation'> 
                    <tbody>
                        <tr style='border-collapse:collapse'> 
                            <td align='center' style='Margin:0;padding-top:15px;padding-bottom:15px;padding-left:25px;padding-right:25px;border-radius:36px'>
                                <p style='Margin:0;font-family:tahoma,verdana,segoe,sans-serif;line-height:30px;color:#1a1a1a;font-size:20px'>
                                    <strong>{code}</strong>
                                </p>
                        </td> 
                        </tr> 
            ...
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine(string.Format(L("PasswordlessCode_ExpirationDescription"), PasswordlessLoginCodeCacheItem.DefaultSlidingExpireTime.TotalMinutes));
            mailMessage.AppendLine("<br />");
            
            await ReplaceBodyAndSendAsync(user.EmailAddress, L("PasswordlessCode_Subject"), emailTemplate,
                mailMessage);
        }
