...
using Seashell.Resort.Authorization.Users.Password;
using Seashell.Resort.Configuration;
using Shouldly;
using Xunit;

namespace Seashell.Resort.Tests.Authorization.Users
{
    public class PasswordExpiration_Tests : AppTestBase
    {
        ...
        [Fact]
        public async Task
            Should_Get_User_Created_Before_Password_Reset_Days_And_Resetted_Password_Before_Password_Reset_Days()
        {
            await EnableResetPasswordFeature(2);
        ...
        [Fact]
        public async Task
            Should_Not_Get_User_Created_Before_Password_Reset_Days_And_Resetted_Password_After_Password_Reset_Days()
        {
            await EnableResetPasswordFeature(2);
        ...
        private async Task<List<User>> CreateTestUserWithUpdatedCreationTime(DateTime creationTime, int loop)
        {
            ...
            await UsingDbContextAsync(async (context) =>
            {
                ...
                await context.UserAccounts.Where(account => userIds.Contains(account.UserId))
                    .ExecuteUpdateAsync(setter => setter.SetProperty(ua => ua.CreationTime, creationTime));
            });
