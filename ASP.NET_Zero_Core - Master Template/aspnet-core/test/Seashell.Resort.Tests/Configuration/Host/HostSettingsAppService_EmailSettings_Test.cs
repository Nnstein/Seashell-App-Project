...
using Seashell.Resort.Configuration.Host;
using Seashell.Resort.Test.Base;
using Shouldly;

namespace Seashell.Resort.Tests.Configuration.Host
{
    ...
    public class HostSettingsAppService_EmailSettings_Test : AppTestBase
    {
        ...
        public async Task Should_Change_Email_Settings()
        {
            ...
            settings.Email.SmtpUseAuthentication.ShouldBe(true);
            ...
            settings.Email.SmtpUseAuthentication = false;
            ...
            (await _settingManager.GetSettingValueAsync<bool>(EmailSettingNames.Smtp.UseDefaultCredentials)).ShouldBe(true); //not changed
        }
    }
}
